...
function Compare-Function{
    ...
    foreach ($prop in $objprops) {
        ...
        if(($RefObject.$prop.GetType().isArray -ne $true) ){
        ...
        elseif(($RefObject.$prop.count -ne $DiffObject.$prop.count) -or `
        ...
        elseif ($RefObject.$prop.GetType().isArray -and ($refObject.$prop | Get-Member -MemberType NoteProperty).count -ne 0 )
        {
